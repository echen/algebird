...
case class BFItem(item: String, hashes: BFHash, width: Int) extends BF {
  ...
  def contains(x: String) = {
    ApproximateBoolean.exact(item == x)
  }
...
case class BFInstance(hashes : BFHash, bits: BitSet, width: Int) extends BF {
   ...
   *
   * Roughly, we can use bounds on the expected number of bits that have been set
   * after n elements have been inserted into the Bloom filter in order to go
   * from the number of bits that have been set (which we know) to an estimate
   * of the cardinality.
   *
   * approximationWidth defines an interval around the maximum-likelihood estimate.
   * Namely, the approximation returned is of the form
   * (min, estimate, max) =
